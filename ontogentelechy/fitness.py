...
from .protocols import AtomSpace, FitnessFunction, Individual
...
class TeleologicalFitness(FitnessFunction):
    ...
    def evaluate(self, individual: Individual, context: Optional[Dict[str, Any]] = None) -> float:
    ...
    def _evaluate_base_fitness(self, individual: Individual) -> float:
        ...
        # Combine
        fitness = float(0.4 * base_fitness + 0.4 * semantic_score + 0.2 * diversity_score)
    ...
    def _evaluate_actualization(self, individual: Individual) -> float:
        ...
        actualization = 0.6 * metrics.actualization + 0.4 * metrics.integration
...
class MultiTelosFitness(FitnessFunction):
    ...
    def __init__(self, atomspace: AtomSpace, teloi: List[tuple[Telos, float]]):
        ...
        self.atomspace = atomspace
        self.teloi = teloi
        self.fitness_functions = [TeleologicalFitness(atomspace, telos) for telos, _ in teloi]
    ...
    def evaluate(self, individual: Individual, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        individual.metadata["multi_telos_scores"] = {
            f"telos_{i}": score for i, score in enumerate(scores)
        }
