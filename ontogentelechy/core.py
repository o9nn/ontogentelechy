...
class Telos:
    ...
    def evaluate_actualization(self, entity: Any) -> float:
        ...
        actualization = float(1.0 / (1.0 + np.exp(-10.0 * (actualization - 0.5))))
...
class ActualizationTracker:
    ...
    def measure_potentiality(self, entity: Any) -> float:
        ...
        if hasattr(entity, "genome") and hasattr(entity.genome, "genes"):
            ...
            active_genes = sum(1 for g in entity.genome.genes if getattr(g, "active", True))
            if total_genes > 0:
    ...
    def measure_emergence(self, entity: Any) -> float:
        ...
        if hasattr(entity, "ontogenetic_state") and hasattr(
            entity.ontogenetic_state, "development_history"
        ):
        ...
        if hasattr(entity, "genome") and hasattr(entity.genome, "fitness"):
            ...
            if hasattr(entity, "metadata") and "previous_fitness" in entity.metadata:
                ...
                emergence = float(np.tanh(improvement * 5.0))  # Scale and bound
    ...
    def _compute_telos_alignment(self, entity: Any, telos: Telos) -> float:
        ...
        if isinstance(target_state, dict):
            ...
            if "weights" in target_state:
                ...
                target_values = np.array(
                    [v for v in target_state.values() if isinstance(v, (int, float))]
                )
        else:
        ...
        if norm_current > 0 and norm_target > 0:
            ...
            alignment = float((similarity + 1.0) / 2.0)
            return alignment
    ...
    def detect_phase_transition(self) -> Optional[Dict[str, Any]]:
        ...
        # Transition criteria
        if actualization_jump > 0.15 or emergence_spike > 0.3 or abs(integration_change) > 0.2:
    ...
    def save(self, path: Union[str, Path]) -> None:
        """Save tracker history to JSON file."""
        data = {
            "history": [
                {
            ...
            "phase_transitions": [
                {k: v for k, v in t.items() if not isinstance(v, ActualizationMetrics)}
    ...
    @classmethod
    def load(cls, path: Union[str, Path]) -> "ActualizationTracker":
        """Load tracker history from JSON file."""
        tracker = cls()
        data = json.loads(Path(path).read_text())
        for m_data in data["history"]:
            tracker.history.append(ActualizationMetrics(**m_data))
        tracker.phase_transitions = data.get("phase_transitions", [])
        return tracker
    ...
    def observe(
        ...
        """Generator that yields ActualizationMetrics on each iteration.

        Usage::

            for metrics in tracker.observe(entity, telos, steps=100):
                # update entity here
                print(metrics.actualization)
        """
        count = 0
        while steps is None or count < steps:
            metrics = self.compute_metrics(entity, telos)
            yield metrics
            count += 1
...
class DevelopmentalAttractor:
    ...
    def compute_gradient(self, current_state: np.ndarray) -> np.ndarray:
        ...
        if distance > 0:
            ...
            return cast(np.ndarray, np.zeros_like(current_state, dtype=np.float64))

        # Compute strength (stronger when further, weaker when close)
        # Use tanh for smooth falloff
        strength = float(np.tanh(distance / self.basin_radius) * self.basin_strength)

        # Gradient points toward attractor
        gradient: np.ndarray = direction * strength
    ...
    def is_in_basin(self, entity: Any) -> bool:
        ...
        if hasattr(entity, "genes"):
            ...
            return bool(distance < self.basin_radius)
